...
def create_mock_row(price: float, sma_20: float, rsi: float, fng: float) -> pd.Series:
...
def test_regime_is_bearish_on_fear():
    ...
    row = create_mock_row(price=90, sma_20=100, rsi=50, fng=FNG_FEAR_THRESHOLD)
...
def test_regime_is_bearish_on_rsi():
    ...
    row = create_mock_row(price=90, sma_20=100, rsi=RSI_BEAR_THRESHOLD, fng=50)
    regime = analyze_market_regime(row)
    assert regime == 'BEARISH'
...
def test_regime_is_bullish():
    ...
    row = create_mock_row(price=110, sma_20=100, rsi=RSI_BULL_THRESHOLD, fng=50)
    regime = analyze_market_regime(row)
...
def test_regime_is_sideways_on_neutral_rsi():
    ...
    row = create_mock_row(price=110, sma_20=100, rsi=RSI_BULL_THRESHOLD - 1, fng=50)
    regime = analyze_market_regime(row)
...
def test_regime_is_sideways_on_price_cross():
    ...
    row = create_mock_row(price=100, sma_20=100, rsi=50, fng=50)
    regime = analyze_market_regime(row)
    assert regime == 'SIDEWAYS'
...
def test_regime_is_sideways_on_conflicting_signals():
    ...
    row = create_mock_row(price=110, sma_20=100, rsi=50, fng=FNG_FEAR_THRESHOLD - 5)
    regime = analyze_market_regime(row)
    assert regime == 'SIDEWAYS'
