...
def save_klines_to_db(df: pd.DataFrame, table_name: str, db_file: str = DB_FILE):
    ...
    if conn:
        try:
            ...
            print(f"Dados salvos/atualizados na tabela '{table_name}' em '{db_file}'")
        except sqlite3.IntegrityError:
            print("Alguns dados já existem (Open_time duplicado), pulando inserção.")
        except sqlite3.Error as e:
...
def get_last_timestamp_from_db(table_name: str, db_file: str = DB_FILE) -> int:
    ...
    if conn:
        try:
            ...
            print(f"Erro ao buscar último timestamp da tabela '{table_name}': {e}")
            return None
        finally:
            conn.close()
    return None
...
def create_open_interest_table(conn: sqlite3.Connection, table_name: str):
    ...
    try:
        ...
        """)
        conn.commit()
        print(f"Tabela '{table_name}' verificada/criada com sucesso para Open Interest.")
    except sqlite3.Error as e:
        print(f"Erro ao criar tabela '{table_name}' para Open Interest: {e}")
...
def save_open_interest_to_db(data: dict, table_name: str, db_file: str):
    ...
    if conn:
        try:
            ...
            """, row)
            conn.commit()
            print(f"Dados de Open Interest salvos/atualizados na tabela '{table_name}'.")
        except sqlite3.Error as e:
            print(f"Erro ao salvar dados de Open Interest no banco de dados: {e}")
        finally:
            conn.close()
...
def create_funding_rate_table(conn: sqlite3.Connection, table_name: str):
    ...
    try:
        ...
        """)
        conn.commit()
        print(f"Tabela '{table_name}' verificada/criada com sucesso para Funding Rate.")
    except sqlite3.Error as e:
        print(f"Erro ao criar tabela '{table_name}' para Funding Rate: {e}")
...
def save_funding_rate_to_db(data: list, table_name: str, db_file: str):
    ...
    if conn:
        try:
            ...
            """, rows)
            conn.commit()
            print(f"Dados de Funding Rate salvos/atualizados na tabela '{table_name}'.")
        except sqlite3.Error as e:
            print(f"Erro ao salvar dados de Funding Rate no banco de dados: {e}")
        finally:
            conn.close()
...
def create_fng_table(conn: sqlite3.Connection, table_name: str):
    ...
    try:
        ...
        """)
        conn.commit()
        print(f"Tabela '{table_name}' verificada/criada com sucesso para F&G Index.")
    except sqlite3.Error as e:
        print(f"Erro ao criar tabela '{table_name}' para F&G Index: {e}")
...
def save_fng_to_db(data: list, table_name: str, db_file: str):
    ...
    if conn:
        try:
            ...
            """, rows)
            conn.commit()
            print(f"Dados do F&G Index salvos/atualizados na tabela '{table_name}'.")
        except sqlite3.Error as e:
            print(f"Erro ao salvar dados do F&G Index no banco de dados: {e}")
        finally:
            conn.close()
...
def get_last_fng_timestamp_from_db(table_name: str, db_file: str) -> int:
    ...
    if conn:
        try:
            ...
            print(f"Erro ao buscar último timestamp do F&G Index: {e}")
            return None
        finally:
            conn.close()
    return None
...
def get_fng_data_from_db(table_name: str, db_file: str, limit: int = None) -> pd.DataFrame:
    ...
    if conn:
        try:
            ...
            print(f"Erro ao carregar dados do F&G Index: {e}")
            return pd.DataFrame()
        finally:
            conn.close()
    return pd.DataFrame()
...
def create_on_chain_table(conn: sqlite3.Connection, table_name: str):
    ...
    try:
        ...
        """)
        conn.commit()
        print(f"Tabela '{table_name}' verificada/criada com sucesso para dados on-chain.")
    except sqlite3.Error as e:
        print(f"Erro ao criar tabela '{table_name}' para dados on-chain: {e}")
...
def save_on_chain_to_db(data: dict, table_name: str, db_file: str):
    ...
    if conn:
        try:
            ...
            """, row)
            conn.commit()
            print(f"Dados on-chain salvos/atualizados na tabela '{table_name}'.")
        except sqlite3.Error as e:
            print(f"Erro ao salvar dados on-chain no banco de dados: {e}")
        finally:
            conn.close()
