...
class RiskManager:
    ...
    def check_health_status(
        ...
        if hf < self.hf_critical:
            return "CRITICAL", hf
        if hf < self.hf_warning:
            return "WARNING", hf
        return "SAFE", hf
    ...
    def adjust_position_size(
        ...
        if adjusted < requested_capital:
            logger.info(
                f"[BALANCE_INFO] Requested: ${requested_capital:.2f} | "
                f"Available: ${available:.2f} | Adjusted: ${adjusted:.2f}"
            )
    ...
    def check_drawdown_limits(
        ...
        if global_hwm > 0:
            ...
            if global_dd >= max_global_dd:
                logger.critical(
                    f"[KILL SWITCH] Global Drawdown {global_dd:.1%} exceeded {regime} limit {max_global_dd:.1%}"
                )
                return "KILL_SWITCH"
        ...
        if daily_hwm > 0:
            ...
            if daily_dd >= self.max_daily_drawdown:
                logger.critical(
                    f"[KILL SWITCH] Daily Drawdown {daily_dd:.1%} exceeded limit {self.max_daily_drawdown:.1%}"
                )
                return "KILL_SWITCH"
    ...
    def assess_rebalance_options(
        ...
        if self.should_emergency_close(balance) and has_active_lps:
            return {
                ...
                "reason": (
                    f"USD balance ${balance:.2f} below emergency threshold "
                    f"(${self.gas_reserve_usd * EMERGENCY_GAS_MULTIPLIER:.2f})"
                ),
                "available_cash": 0.0,
            }
